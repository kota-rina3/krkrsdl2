...
public:
	...
	virtual void __stdcall SetPosition(unsigned __int64 tick) override
		{ SeekToMs((double)tick); }
	virtual void __stdcall GetPosition(unsigned __int64 *tick) override
		{ if(tick) *tick = (unsigned __int64)GetClockMs(); }
	...
	virtual void __stdcall GetNumberOfFrame( int *f ) override
		{ if(f) *f = NumFrames; }
	virtual void __stdcall GetTotalTime( __int64 *t ) override
		{ if(t) *t = TotalTimeMs; }
